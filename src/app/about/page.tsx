  ...
  return (
    <section className="shell section-space">
      <div className="soft-grid relative overflow-hidden rounded-[2.5rem] border border-white/60 bg-white/30 px-6 py-8 sm:px-8 sm:py-10 lg:px-10 lg:py-12">
        <div className="hero-glow hero-glow-primary left-[-10%] top-[-12%] h-[14rem] w-[14rem] sm:h-[20rem] sm:w-[20rem]" />
      ...
      <div className="mt-14 grid gap-5 lg:grid-cols-2">
        <section className="panel p-8">
          ...
          <StaggerGroup className="mt-5 flex flex-wrap gap-3">
            {focuses.map((focus) => (
              <StaggerItem key={focus}>
                <span className="rounded-full border border-line bg-white/50 px-4 py-2 text-sm text-foreground">{focus}</span>
              </StaggerItem>
            ))}
          </StaggerGroup>
        </section>
